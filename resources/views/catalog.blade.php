    ...
    <div class="catalog_sort my-3">
        <div class="dropdown float-md-start">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                @foreach($categories as $category)<!--фильтр по категориям-->
                <a class="dropdown-item" href="{{url('catalog/category')}}/{{$category->id}}" role="button">{{ $category->name }}</a>
                @endforeach
            </div>
        </div>

        <div class="my-3 float-md-end">
            <a href="{{ url('/catalog') }}?sort_by=name&sort_order={{ $sort_by == 'name' && $sort_order == 'asc' ? 'asc' : 'asc' }}">Сортировка по имени</a> |
            <a href="{{ url('/catalog') }}?sort_by=price&sort_order={{ $sort_by == 'price' && $sort_order == 'asc' ? 'desc' : 'asc' }}">Сортировка по цене</a>
        </div>
    </div>
